...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if ((self = [super initWithFrame:frame])) {
        ...
        self.action = action;
        self.status = RefreshCoreTextStatusHidden;
        self.loading = NO;
    ...
    if (pan.state == UIGestureRecognizerStateChanged) {
        ...
        if (offset <= 0) {
            ...
            if (fractionDragged >= 1) {
                self.status = RefreshCoreTextStatusTriggered;
            } else {
                self.status = RefreshCoreTextStatusDragging;
            }
        ...
        if (self.status == RefreshCoreTextStatusTriggered) {
            [self startLoading];
